...
- `raspberry/install.sh` - Added nginx permissions fix during installation
- `raspberry/scripts/setup-remote-club.sh` - Added nginx permissions fix during remote setup
...
This fix addresses multiple issues discovered during new device setup:

### 1. Socket.IO Connection Failure
- Installation script (`setup.sh`) completed successfully
- Sync-agent was installed and started
- But connection to central server failed with "websocket error"
- Device appeared offline in dashboard
- Commands from dashboard couldn't reach the device

### 2. Nginx Permission Error (500 Internal Server Error)
During testing, we also discovered that nginx couldn't serve the Angular application due to permission issues:

**Symptom:**
- `/tv` and `/remote` routes returned "500 Internal Server Error"
- Nginx logs showed: `stat() "/home/pi/neopro/webapp/index.html" failed (13: Permission denied)`

**Root Cause:**
- The `/home/pi` directory had restrictive permissions (`700` / `drwx------`)
- Nginx (running as `www-data`) couldn't traverse to `/home/pi/neopro/webapp/`

**Fix Applied:**
Added permission configuration in both installation scripts:
```bash
chmod 755 /home/pi
chmod 755 /home/pi/neopro
chmod -R 755 /home/pi/neopro/webapp
chown -R pi:www-data /home/pi/neopro/webapp
```

This ensures nginx can serve the application files on first installation.
